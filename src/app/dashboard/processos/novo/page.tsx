...
export default function NovoProcessoPage() {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        const filePath = `processos/${pageProcessId}/${Date.now()}-${sanitizeFileName(file.name)}`;
        ...
        setFormData((prev: any) => {
            ...
            if (targetObj?.type === 'nota_fiscal') return { ...prev, notas_fiscais: prev.notas_fiscais.map((nf: any) => nf.id === targetObj.id ? { ...nf, file: placeholder } : nf) };
            if (targetObj?.type === 'documento_pos_embarque') return { ...prev, documentos_pos_embarque: prev.documentos_pos_embarque.map((d: any) => d.id === targetObj.id ? { ...d, file: placeholder } : d) };
            if (targetObj?.type === 'documento_fiscal') return { ...prev, documentos_fiscais: (prev.documentos_fiscais || []).map((df: any) => df.id === targetObj.id ? { ...df, file: placeholder } : df) };
            return prev;
        });
        ...
        uploadTask.on('state_changed',
            (snap) => setUploadProgresses(prev => ({ ...prev, [filePath]: (snap.bytesTransferred / snap.totalBytes) * 100 })),
            ...
            async () => {
                ...
                setFormData((prev: any) => {
                    ...
                    else if (targetObj?.type === 'nota_fiscal') newState.notas_fiscais = prev.notas_fiscais.map((nf: any) => nf.id === targetObj.id ? { ...nf, file: finalFileData } : nf);
                    else if (targetObj?.type === 'documento_pos_embarque') newState.documentos_pos_embarque = prev.documentos_pos_embarque.map((d: any) => d.id === targetObj.id ? { ...d, file: finalFileData } : d);
                    else if (targetObj?.type === 'documento_fiscal') newState.documentos_fiscais = prev.documentos_fiscais.map((df: any) => df.id === targetObj.id ? { ...df, file: finalFileData } : df);
                    
                    if (firestore && pageProcessId) {
                        const syncData: any = {};
                        if (targetField) syncData[targetField] = finalFileData;
                        else {
                            const key = targetObj?.type === 'nota_fiscal' ? 'notas_fiscais' : targetObj?.type === 'documento_pos_embarque' ? 'documentos_pos_embarque' : 'documentos_fiscais';
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Accordion type="single" collapsible className="w-full" defaultValue="item-1">
          ...
          <AccordionItem value="item-3" disabled={!isEditing}>
            <AccordionTrigger><div className='flex items-center gap-3'>{getStepStatusIcon(3)}<h3 className="text-lg font-semibold">Etapa 3: Fiscal e Tratamento</h3></div></AccordionTrigger>
            <AccordionContent>
              <Card><CardContent className="space-y-6 pt-6">
                <div className="flex justify-between items-center"><h3 className="font-medium">Documentos Fiscais e Tratamento</h3><Button variant="outline" size="sm" type="button" onClick={() => setFormData((prev: any) => ({ ...prev, documentos_fiscais: [...(prev.documentos_fiscais || []), { id: Date.now(), tipo: 'DUE', identificacao: '', status: 'RASCUNHO SALVO', data: null, file: null }] }))}><PlusCircle className="mr-2 h-4 w-4" />Adicionar</Button></div>
                ...
                <TableBody>{formData.documentos_fiscais?.map((df: any) => (
                  <TableRow key={df.id}>
                    ...
                    <TableCell><div className="flex items-center gap-2"><div className="flex-1 p-2 border rounded-md text-[10px] bg-muted overflow-hidden">{renderFileState(df.file)}</div>{df.file ? <Button variant="ghost" size="icon" type="button" onClick={() => removeFile({ type: 'documento_fiscal', id: df.id })}><Trash2 className="h-3 w-3" /></Button> : <Button variant="outline" size="icon" type="button" onClick={() => triggerFileUpload({ type: 'documento_fiscal', id: df.id })}><Upload className="h-3 w-3" /></Button>}</div></TableCell>
