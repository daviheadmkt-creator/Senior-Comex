...
const staticProcessos = [
    ...
    { id: '4', status: 'Aguardando embarque' },
    { id: '5', status: 'Em trânsito' },
];

export default function DashboardPage() {
    const totalOperacoes = staticProcessos.length;
    const alertasAtuais = staticProcessos.filter(p => p.status === 'Atrasado').length;
    const concluidosMes = staticProcessos.filter(p => p.status === 'Concluído').length; 

    const statusData = staticProcessos.reduce((acc, processo) => {
